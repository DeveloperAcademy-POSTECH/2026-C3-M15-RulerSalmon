...
- `LanguageModelSession` 기반의 세션형 호출 구조
- 프롬프트 응답뿐 아니라 구조화된 출력 생성 지원
- 네트워크 비의존형 동작 구조
- 사용자 데이터가 디바이스 외부로 나가지 않는 로컬 추론 구조

## 이 앱에서 Foundation Models가 필요한 이유
- 회고 대화에서 자연스러운 후속 질문 생성 필요성
- 고정 규칙만으로 처리하기 어려운 발화 해석 보강 필요성
- 4L 분류 결과를 그대로 노출하지 않고 사용자 친화적으로 정제할 필요성
- 리포트 단계에서 회고 내용을 한 줄 요약, 정리 문장, 액션 아이템 형태로 변환할 필요성

## 이 앱에서의 사용 역할
- 회고 채팅 단계의 질문 생성 엔진 역할
- 1차 4L 분류 결과의 2차 검토 계층 역할
- 리포트 문장 정제 계층 역할
- 회고 요약 생성 계층 역할
...
## 정리
Foundation Models는 이 앱에서 단순 챗봇 엔진이 아니라, 회고 맥락을 읽고 다음 질문을 생성하며 리포트 문장을 정제하는 핵심 생성 계층
